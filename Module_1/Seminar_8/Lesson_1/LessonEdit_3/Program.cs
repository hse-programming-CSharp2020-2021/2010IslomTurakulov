...
1) Создайте копию проекта и внесите следующие изменения в программу: генерируемое в пункте 1 предложение может не заканчиваться точкой, поэтому в пункте 3 необходимо проверять стоит ли удалять последний символ из предложения
...
class Program
{
    ...
    static void Main()
    {
        ...
        for (int i = 0; i < linesCount; i++)
        {
            ...
            File.WriteAllText(fileName, message, enc); // Добавляем строку в файл.
        ...
        foreach (string item in messages)
            if (item[^1] == '.')
                File.ReadAllText(item[0..^1]); // Выводим сообщения из переписки без точек на экран.
    }
}